...
function setup() {
 ...
 //criar uma sprite para o solo
  ...
  fimDeJogo.scale = width/height*0.5;
  fimDeJogo.visible = false;
  fimDeJogo.depth = trex.depth + 20;
  restart = createSprite(width/2, 150);
  restart.addImage(restartimg);
  restart.scale = width/height*0.5;
  restart.visible = false;
  restart.depth = trex.depth + 20;
...
function draw() {
  ...
  drawSprites();
  text("Pontuação: " + pontuacao + " seg", (width * 3) / 5, 20);
  //text(""+trex.y, (width * 3)/5, 40);
}
...
function geranuvem() {
  if (frameCount % 60 === 0) {
    var nuvem = createSprite(width, 150, 40, 10);
    nuvem.velocityX = -3;
    ...
    //dimensiona:
    //nuvem.scale = 0.4;
    nuvem.scale = width/height*random(0.25, 0.6);

    //profundidade:
    nuvem.depth = trex.depth - 10;
...
function geraObstaculos() {
  if (frameCount % 60 === 0) {
    var obstaculo = createSprite(width, 178, 10, 40);
